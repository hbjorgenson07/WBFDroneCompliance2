...
import { buildIndianaComplianceCsv } from '@/lib/compliance'
import { SprayLog } from '@/lib/types'

// GET /api/export/compliance — Indiana compliance CSV download
export async function GET(request: NextRequest) {
  ...
  const csv = buildIndianaComplianceCsv(data as SprayLog[])
  const filename = `indiana-compliance-${new Date().toISOString().slice(0, 10)}.csv`
